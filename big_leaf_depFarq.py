...
from radiation import calculate_absorbed_radiation_big_leaf
...
class CoupledModel(object):
    ...
    def main(self, tair, par, vpd, wind, pressure, Ca, doy, hod, lat, lon,
        ...
        if elevation > 0.0 and par > 50.0:
            ...
            (apar,
             lai_leaf,
             kb) = calculate_absorbed_radiation_big_leaf(par, cos_zenith, lai,
                                                         direct_frac,
                                                         diffuse_frac)
            ...
            iter = 0
            while True:
                (An, gsc) = F.photosynthesis(Cs=Cs, Tleaf=Tleaf_K,
                                            Par=apar,
                                            Jmax25=self.Jmax25,
...
def calc_leaf_to_canopy_scalar(lai, kb):
    ...
    * Wang and Leuning (1998) AFm, 91, 89-111; particularly the Appendix.
    ...
    # Parameter to scale up from single leaf to the big leaves
    scalex = lai * (1.0 - exp(-kn)) / kn
